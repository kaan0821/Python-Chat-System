...
# ==============================================================================
                ...
                for g in the_guys:
                    ...
                    ctime = time.strftime('%d.%m.%y,%H:%M', time.localtime())
                    better_message = '(' + from_name + ')' + ctime + ' ' + message
                    self.indices[g].add_msg_and_index(better_message)
                    msg = json.dumps(
                            {'action':'exchange','from':from_name,'message':better_message})
                    mysend(
                        to_sock,msg)
...
# ==============================================================================
                ...
                # ---- start your code ---- #
                target = msg['target']
                poem = self.sonnet.get_poem(int(target))
                # ---- end of your code --- #
                mysend(from_sock, json.dumps(
...
# ==============================================================================
                ...
                # ---- start your code ---- #
                target = msg['target']
                search_name = self.logged_sock2name[from_sock]
                search_rslt=self.indices[search_name].search(target)
                search_string=''
                for i in search_rslt:
                    search_string += str(i)
                #print('server side search: ' + search_string)
                # ---- end of your code --- #
                mysend(from_sock, json.dumps(
                    {"action": "search", "results": search_string}))
